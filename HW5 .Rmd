...
```{r}
# Enter your name here: Sana Khan| Homework 5 | Due 11.15.2021
```
...
#The variables were measured once a year for each country 
...
```{r}
#The code above is finding all of the NA values for the new_sp_f04. The is.na will show any NA values in the attribute that is specified 
```
...
tbCan$new_sp_f654[is.na(tbCan$new_sp_f65)]
tbCan$new_sp_m65[is.na(tbCan$new_sp_m65)]

#Observations increase as age increases
...
tbCan$new_sp_f654 <- na_interpolation(tbCan$new_sp_f65)
...
tbCan %>%
ggplot() +
geom_histogram(aes(x=new_sp_m014), fill="green",color = "grey", alpha=0.5, bins =5) +
xlab("Cases per Year") + 
ylab("Count of Cases") +
ggtitle("Pulmonary smear positive cases: Males 0-14")
#This Histogram is is showing the frequency of postive cases per year for males 0-14. 
...
tbCan %>%
ggplot() +
geom_histogram(aes(x=new_sp_f014), fill="green",color = "grey", alpha=0.5, bins =5) +
xlab("Cases per Year") + 
ylab("Count of Cases") +
ggtitle("Pulmonary smear positive cases: Females 0-14")

tbCan %>%
ggplot() +
geom_histogram(aes(x=new_sp_f65), fill="green",color = "grey", alpha=0.5, bins =10) +
xlab("Cases per Year") + 
ylab("Count of Cases") +
ggtitle("Pulmonary smear positive cases: Females 65+")

tbCan %>%
ggplot() +
geom_histogram(aes(x=new_sp_m65), fill="green",color = "grey", alpha=0.5, bins =10) +
xlab("Cases per Year") + 
ylab("Count of Cases") +
ggtitle("Pulmonary smear positive cases: Males 65+")
# I adjusted the number of bins to try ot show the range a bit more clearly 
```
...
tbCan %>%
ggplot() +
geom_line(aes(x=year, y=new_sp_f014),color = "pink") 


tbCan %>%
ggplot() +
geom_line(aes(x=year, y=new_sp_f65),color = "red") 

tbCan %>%
ggplot() +
geom_line(aes(x=year, y=new_sp_m65),color = "blue") 
...
```{r}
tbCan %>%
ggplot(aes(x=new_sp_combined014, y = new_sp_combined65)) + 
...
```{r}
#This is showing that as the years increase, the number of positive tests decrease for both age groups. There is a higher concentration in the lower left quadrant, which also coresponds to 2000 and on  
```
